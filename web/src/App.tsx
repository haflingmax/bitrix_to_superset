...
const App: React.FC = () => {
  ...
  const fetchSyncCounts = async () => {
    try {
      const response = await axios.get<{ [key: string]: SyncCounts }>("/sync_counts");
  ...
  const fetchSyncStatus = async () => {
    try {
      const response = await axios.get<{ [key: string]: SyncStatus }>("/sync_status");
  ...
  const confirmSync = async () => {
    ...
    try {
      await axios.post(`/sync/${syncPrompt}`);
      setSyncPrompt(null);
    } catch (err) {
